...
Unity 受信・入力・ジェスチャーの正本は[arm-strike-game の仕様](../../../arm-strike-game/.scratch/arm-strike-docs/spec.md)に置く。この文書は Kinect、人物追跡、UDP/JSON、Bridge 運用の正本であり、ゲーム仕様は扱わない。
...
- `sessionId` は Bridge 起動時、`frameId` と `timestampMs` は連番で送られる。人物がいないフレームは `tracked:false` だが、センサー切断時には状態パケットを送らない。
...
- Kinect 切断時は最大10秒自動再接続を待ち、失敗時は Bridge 再起動、次に適合済み予備 Kinect へ交換する。Unity 再起動は受信側障害が確認された場合のみ行う。
...
- Windows 11 のエディション・OS ビルド、Kinect ドライバー詳細、.NET Framework 4.8 Release 値、USB 構成、モデル/シリアル、電源アダプター、USB 変換ケーブル、接続ポート、個体別適合結果は未確認。
- Kinect for Xbox 360 を使用予定。使用・予備を含む個体ごとの適合試験に合格した構成のみを対応環境として記録する。
